...
namespace Event_Creator.Controllers
{
...

    {
        ...
        {
            string path = null;
            if (kind == Image.book) path = Path.GetFullPath(Path.Combine(@"..\Resources\webApi\images\books"));
            else path = Path.GetFullPath(Path.Combine(@"..\Resources\webApi\images\users"));
        ...
        public async Task<IActionResult> UploadProfile(IFormFile profile)
        {
            ...
            var path = Path.GetFullPath(Path.Combine(@"..\Resources\webApi\images\users",name));
        ...
        public async Task<IActionResult> AddBookImage(IFormFile image,long bookId)
        {
            ...
            var path = Path.GetFullPath(Path.Combine(@"..\Resources\webApi\images\books", name));
            var fileStream = new FileStream(path, FileMode.Create);
        ...
        public async Task<IActionResult> DeleteBookImage(long bookId, int imageId)
        {
            ...
            var path = Path.GetFullPath(Path.Combine(@"..\Resources\webApi\images\books"));
        ...
        {
            long userId = _jwtService.getUserIdFromJwt(HttpContext);
            var path = Path.GetFullPath(Path.Combine(@"..\Resources\webApi\images\users"));
